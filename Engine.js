...
export class Engine {
    ...
    handleEndOfDay() {
        ...
        for(let y = 0; y < CONFIG.GRID_H; y++) {
            for(let x = 0; x < CONFIG.GRID_W; x++) {
                ...
                if (cell.isRoot) {
                    ...
                    // NEW: Only collect condo rent from occupied units
                    if (cell.type === TYPES.CONDO && cell.occupied) {
                        dailyIncome += CONFIG.CONDO_RENT;
                        console.log(`Collected $${CONFIG.CONDO_RENT} rent from occupied condo at (${x}, ${y})`);
                    }
                }
            }
        }
        ...
        // Log condo occupancy stats
        const totalCondos = this.getRoomCount(TYPES.CONDO);
        const occupiedCondos = this.getOccupiedRoomCount(TYPES.CONDO);
        console.log(`Day ${this.day}: ${occupiedCondos}/${totalCondos} condos occupied`);
    }
    ...
    getRoomCount(type) {
        ...
        for(let y = 0; y < CONFIG.GRID_H; y++) {
            for(let x = 0; x < CONFIG.GRID_W; x++) {
                if (this.gridManager.grid[y][x].type === type && this.gridManager.grid[y][x].isRoot) {
                    count++;
                }
            }
        }
    ...
    getOccupiedRoomCount(type) {
        let count = 0;
        for(let y = 0; y < CONFIG.GRID_H; y++) {
            for(let x = 0; x < CONFIG.GRID_W; x++) {
                const cell = this.gridManager.grid[y][x];
                if (cell.type === type && cell.isRoot && cell.occupied) {
                    count++;
                }
            }
        }
